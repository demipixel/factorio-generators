...
module.exports = function(string, opt = {}) {
  ...
  if (INCLUDE_TRAIN_STATION) {
    ...
    for (let l = 0; l < LOADING_BAYS; l++) {
      ...
      if (l == 0) {
        RAIL_X = OFFSET_X;
        trainStopLocation = generateTrainStation(bp, { x: OFFSET_X, y: OFFSET_Y }, START_TO_CARGO + ((SINGLE_HEADED_TRAIN ? 0 : 1) * LOCOMOTIVES) + (LOADING_BAYS * (LOAD_FROM_BOTH_SIDES ? 2 : 1)), {
          LOCOMOTIVES,
          TRACK_CONCRETE,
          SINGLE_HEADED_TRAIN,
          WALL_SPACE,
